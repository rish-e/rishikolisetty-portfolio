...
import { Code2, Globe, Server, Brain, Cloud, Wrench } from "lucide-react";
...
const categories = [
  ...
  { label: "AI & ML", icon: Brain, color: "bg-purple-500", items: ["Claude API", "LLMs", "MCP", "AI SDK"] },
  { label: "Infrastructure", icon: Cloud, color: "bg-cyan-500", items: ["Vercel", "Supabase", "Cloudflare", "Docker"] },
  { label: "Tools", icon: Wrench, color: "bg-gray-500", items: ["FFmpeg", "Puppeteer", "Playwright", "Git"] },
];
                ...
                <div className="mt-4 space-y-2">
                  {cat.items.map((item) => (
                    <div key={item} className="flex items-center justify-between">
                      <span className="text-[13px] text-[#aaa]">{item}</span>
                      <div className="flex h-[20px] w-[36px] items-center rounded-full bg-[#34c759] px-0.5">
                        <div className="ml-auto h-[16px] w-[16px] rounded-full bg-white shadow-sm" />
                      </div>
                    </div>
                  ))}
                </div>
